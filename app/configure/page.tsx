...
export default function ConfigurePage() {
  ...
  const inputCls =
    "w-full rounded-md bg-[#11161f] px-3 py-2.5 text-sm text-neutral-100 outline-none border border-[#2a3142] focus:border-[#C9A96E] focus-visible:ring-2 focus-visible:ring-[#C9A96E] placeholder:text-neutral-500";

  return (
    <div className="min-h-screen bg-[#0f1419] text-neutral-200">
      <div className="mx-auto max-w-7xl px-4 pb-12 pt-28 sm:px-6 lg:px-8">
        <header className="mb-8 border-b border-[#2a3142] pb-6">
          <div className="mb-3 flex items-center gap-2">
            <span className="h-1.5 w-1.5 rounded-full bg-[#C9A96E]" />
            <span className="text-xs font-semibold tracking-wide text-[#C9A96E]">Interactive designer</span>
          </div>
          <h1 className="font-[family-name:var(--font-playfair)] text-3xl font-bold tracking-tight text-white sm:text-4xl">
      ...
      {pending && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={close}>
          <div className="w-full max-w-md rounded-xl border border-[#2a3142] bg-[#1a1f2e] p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            {status === "done" ? (
              <div className="text-center">
                ...
                <button onClick={close} className="mt-6 w-full rounded-md bg-[#C9A96E] py-2.5 text-sm font-bold text-[#0f1419] transition-colors hover:bg-[#b8954f]">
                  Done
                </button>
              </div>
            ) : (
              <form onSubmit={submit}>
                ...
                <button type="submit" disabled={status === "sending"} className="mt-5 w-full rounded-md bg-[#C9A96E] py-3 text-sm font-bold text-[#0f1419] transition-colors hover:bg-[#b8954f] disabled:opacity-60">
